...
可以在`SharedModule`程序集中的`Implement`目录下查看`ManagerBase.cs`。
...
其中`SharedModule`是一个共享模块，其他各个模块都会引用它。
...
在`Program.cs`中，主要就是用来注入服务、配置中间件以及初始化操作。通常代码如下:
...
using (app)
{
    ...
    using (IServiceScope scope = app.Services.CreateScope())
    {
        ...
        Initialize.Check(provider);
    }
    app.Run();
}
```

其中`AddManagers`和`AddModules`方法是用来添加业务模块的服务。它们由源代码生成器自动生成，无法手支修改，可以按`F12`查看生成的代码。
